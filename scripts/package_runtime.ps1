...
$ModelPackages = @("DG_8_weights", "DG_10_weights")
foreach ($ModelPackage in $ModelPackages) {
    $ModelSourceDir = Join-Path $AppDir "models\$ModelPackage"
    Require-File (Join-Path $ModelSourceDir "best_obb.xml") "$ModelPackage OBB model XML"
    Require-File (Join-Path $ModelSourceDir "best_obb.bin") "$ModelPackage OBB model BIN"
    Require-File (Join-Path $ModelSourceDir "best_seg.xml") "$ModelPackage SEG model XML"
    Require-File (Join-Path $ModelSourceDir "best_seg.bin") "$ModelPackage SEG model BIN"
}
...
foreach ($ModelPackage in $ModelPackages) {
    $ModelSourceDir = Join-Path $AppDir "models\$ModelPackage"
    $ModelDestinationDir = Join-Path $StagingDir "models\$ModelPackage"
    foreach ($ModelFile in @("best_obb.xml", "best_obb.bin", "best_seg.xml", "best_seg.bin")) {
        Copy-File (Join-Path $ModelSourceDir $ModelFile) $ModelDestinationDir
    }
}
...
$ProjectProfilesSource = Join-Path $BuildOutputPath "config\project_profiles"
if (-not (Test-Path -LiteralPath $ProjectProfilesSource)) {
    $ProjectProfilesSource = Join-Path $AppDir "config\project_profiles"
}
Copy-DirectoryIfExists $ProjectProfilesSource (Join-Path $StagingDir "config\project_profiles")
$AdminAuthKeySource = Join-Path $AppDir "config\admin_auth.key"
if (-not (Test-Path -LiteralPath $AdminAuthKeySource)) {
    throw "Missing config\admin_auth.key. Formal packages must include the administrator authorization key."
}
...
$ObbModel = Join-Path $AppDir "models\DG_8_weights\best_obb.xml"
$SegModel = Join-Path $AppDir "models\DG_8_weights\best_seg.xml"
