                         ...
                         const PlcOutputConfig& plc_config)
{
    for (int i = 0; i < static_cast<int>(result.detections.size()); ++i) {
        ...
        detection.has_display_angle = false;
        detection.display_angle_deg = 0.0f;
        detection.has_display_geometry_angle_offset = false;
        detection.display_geometry_angle_offset_deg = 0.0f;
        ...
        const HeadTypeCompensation& type_compensation =
            CompensationForHeadType(plc_config, detection.head_type_code);
        detection.display_angle_deg =
            ApplyPlcAngleCalibration(detection.angle_deg, plc_config, type_compensation);
        detection.has_display_angle = true;
        detection.display_geometry_angle_offset_deg = type_compensation.angle_offset_deg;
        detection.has_display_geometry_angle_offset =
            std::fabs(type_compensation.angle_offset_deg) > 1e-6f;
