...
public:
    ...
    void setCenterRayOffsetPx(double offset_px);
    double centerRayOffsetPx() const;
    void setPostprocessDebugLoggingEnabled(bool enabled);
    bool postprocessDebugLoggingEnabled() const;
    void setCameraIp(const std::string& camera_ip);
    std::string cameraIp() const;
    void setCameraExposureUs(double exposure_us);
    double cameraExposureUs() const;
    bool autoExposeOnce(double* exposure_us, std::string* error_message);

    bool startFrameCapture(std::function<void(const cv::Mat&)> on_frame,
