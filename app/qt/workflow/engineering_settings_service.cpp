...
QString EngineeringSettingsService::LoadCameraIp(const QString& default_ip)
{
    auto settings = CreateSettings();
    settings->beginGroup(QStringLiteral("camera"));
    const QString ip = settings->value(QStringLiteral("ip"), default_ip).toString().trimmed();
    settings->endGroup();
    return ip;
}

void EngineeringSettingsService::SaveCameraIp(const QString& ip)
{
    auto settings = CreateSettings();
    settings->beginGroup(QStringLiteral("camera"));
    settings->setValue(QStringLiteral("ip"), ip.trimmed());
    settings->endGroup();
    settings->sync();
}
...
AngleCalibrationSettings EngineeringSettingsService::LoadAngleCalibrationSettings(const AngleCalibrationSettings& defaults)
{
    ...
    const double legacy_offset = settings->value(QStringLiteral("offset_deg"), angle.offset_deg).toDouble();
    ...
    angle.offset_deg = legacy_offset;
    settings->endGroup();
    return angle;
}
...
void EngineeringSettingsService::SaveAngleCalibrationSettings(const AngleCalibrationSettings& angle)
{
    auto settings = CreateSettings();
    settings->beginGroup(QStringLiteral("angle_calibration"));
    settings->setValue(QStringLiteral("offset_deg"),
                       angle.offset_deg);
    ...
    settings->remove(QStringLiteral("zero_to_360_offset_deg"));
    settings->remove(QStringLiteral("signed_180_offset_deg"));
    settings->remove(QStringLiteral("has_zero_to_360_offset"));
    settings->remove(QStringLiteral("has_signed_180_offset"));
    settings->remove(QStringLiteral("raw_angle_deg"));
    settings->remove(QStringLiteral("target_angle_deg"));
    settings->remove(QStringLiteral("has_anchor"));
    settings->endGroup();
    settings->sync();
}
...
ObbPostprocessSettings EngineeringSettingsService::LoadObbPostprocessSettings()
{
    ...
    postprocess.show_head_type_adjusted_geometry =
        settings->value(QStringLiteral("show_head_type_adjusted_geometry"), true).toBool();
...
void EngineeringSettingsService::SaveObbPostprocessSettings(const ObbPostprocessSettings& postprocess)
{
    ...
    settings->setValue(QStringLiteral("show_head_type_adjusted_geometry"),
                       postprocess.show_head_type_adjusted_geometry);
