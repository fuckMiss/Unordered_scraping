...
void DrawDetectionPolygon(Mat& image,
                          ...
                          bool show_head_type_adjusted_geometry,
                          double display_scale)
{
    ...
    const float geometry_angle_delta_deg = show_head_type_adjusted_geometry && detection.has_display_geometry_angle_offset
        ? detection.display_geometry_angle_offset_deg
        : 0.0f;
    vector<Point2f> display_gripper_corners = detection.mechanical_gripper_corners;
    if (use_command_pose || std::fabs(geometry_angle_delta_deg) > 0.01f) {
        for (Point2f& point : display_gripper_corners) {
            point = RotatePointAroundCenter(point, raw_center, geometry_angle_delta_deg) + command_delta;
        }
    }
    ...
    const Point2f arrow_start =
        RotatePointAroundCenter(detection.arrow_start, raw_center, geometry_angle_delta_deg) + command_delta;
    const Point2f arrow_end =
        RotatePointAroundCenter(detection.arrow_end, raw_center, geometry_angle_delta_deg) + command_delta;
    ...
    if (show_all_detections) {
        ...
        for (int i = 0; i < static_cast<int>(result.detections.size()); ++i) {
            ...
            DrawDetectionPolygon(image,
                                 ...
                                 true,
                                 false,
                                 show_head_type_adjusted_geometry,
