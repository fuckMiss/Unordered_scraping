...
struct StandardModelFolderSelection
{
    QString obb_xml_path;
    QString seg_xml_path;
    QStringList missing_files;

    bool complete() const
    {
        return missing_files.isEmpty() && !obb_xml_path.isEmpty() && !seg_xml_path.isEmpty();
    }
};
...
struct EngineeringSettingsControls
{
    ...
    QLineEdit* obb_model_path_edit = nullptr;
    QLineEdit* seg_model_path_edit = nullptr;
    ...
    QCheckBox* show_head_type_adjusted_geometry_check = nullptr;
...
struct EngineeringSettingsDraft
{
    ...
    bool show_head_type_adjusted_geometry = true;
