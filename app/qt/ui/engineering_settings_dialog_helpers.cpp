...
StandardModelFolderSelection DetectStandardModelFolder(const QString& folder_path)
{
    StandardModelFolderSelection result;
    const QDir folder(folder_path.trimmed());
    ...
    for (const QString& file_name : required_files) {
        if (!folder.exists(file_name)) {
            result.missing_files.append(file_name);
        }
    }
    if (result.missing_files.isEmpty()) {
        result.obb_xml_path = QFileInfo(folder.filePath(QStringLiteral("best_obb.xml"))).absoluteFilePath();
        result.seg_xml_path = QFileInfo(folder.filePath(QStringLiteral("best_seg.xml"))).absoluteFilePath();
    }
    return result;
}
                                  ...
                                  EngineeringSettingsValidationError* error)
{
    ...
    next_draft.obb_model_path = controls.obb_model_path_edit != nullptr
        ? controls.obb_model_path_edit->text().trimmed()
        : QString();
    next_draft.seg_model_path = controls.seg_model_path_edit != nullptr
        ? controls.seg_model_path_edit->text().trimmed()
        : QString();
    ...
    next_draft.show_head_type_adjusted_geometry =
        controls.show_head_type_adjusted_geometry_check->isChecked();
    ...
    const auto range_mode = static_cast<AngleRangeMode>(controls.angle_range_combo->currentData().toInt());
    next_draft.angle_range_mode = range_mode;
