...
void PlcRelatedSettingsRoundTrip()
{
    ...
    QSettings legacy_angle_settings(QString::fromLocal8Bit(qgetenv("TANKEYE_SETTINGS_INI_PATH")),
                                   QSettings::IniFormat);
    legacy_angle_settings.beginGroup(QStringLiteral("angle_calibration"));
    legacy_angle_settings.setValue(QStringLiteral("offset_deg"), -12.5);
    legacy_angle_settings.setValue(QStringLiteral("direction"), QStringLiteral("reverse"));
    legacy_angle_settings.setValue(QStringLiteral("range_mode"), QStringLiteral("signed_180"));
    legacy_angle_settings.endGroup();
    legacy_angle_settings.sync();

    const AngleCalibrationSettings loaded_legacy_angle =
        EngineeringSettingsService::LoadAngleCalibrationSettings();
    assert(loaded_legacy_angle.reverse_direction);
    assert(loaded_legacy_angle.range_mode == AngleRangeMode::Signed180);
    assert(NearlyEqual(loaded_legacy_angle.offset_deg, -12.5));
    ...
    assert(EngineeringSettingsService::LoadObbPostprocessSettings().show_head_type_adjusted_geometry);

    ObbPostprocessSettings postprocess;
    postprocess.center_ray_offset_px = 12.0;
    postprocess.show_head_type_adjusted_geometry = false;
    ...
    assert(!loaded_postprocess.show_head_type_adjusted_geometry);
