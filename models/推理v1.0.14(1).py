...
def visualize_group_aligned(img_path):
    ...
    if len(groups) > 0:
        for g_idx, g in enumerate(groups):
            ...
            pts4_raw = np.array(r_box_8).reshape(4, 2)
            aligned_pts4 = rotate_polygon_optimal(pts4_raw, ref_angle)
            aligned_8 = aligned_pts4.reshape(-1)
            aligned_center_A = get_obb_center(aligned_8)

            # 先对【对齐角度后的原始红色框】求垂足、筛选最优AC向量
            foot_points = get_perpendicular_foot_points(aligned_pts4, aligned_center_A)
            vec_OA = aligned_center_A - center_O
            candidate_list = []
            for idx, (foot_C, _) in enumerate(foot_points):
                vec_AC = foot_C - aligned_center_A
            ...
            # 第二步：沿垂直AC方向拉伸红色OBB，A保持中心
            scaled_4pts = scale_obb_along_perp_AC(aligned_pts4, aligned_center_A, best_vec_AC, PERP_SCALE)
            scaled_8 = scaled_4pts.reshape(-1)
            scaled_center_A = aligned_center_A  # 拉伸后A不变
