...
- 2026-08-13: 已实现夹爪框整框保护区硬约束。`ApplyMechanicalGripperCollisionFilter()` 现在复用 `BuildGrabLimitOverlayPolygon()` 生成的保护区多边形作为唯一安全边界来源，并要求实际 `mechanical_gripper_corners` 四角全部严格在保护区内部；任一角越界、压线、夹爪框缺失或保护区无效都会置 `can_grab=false`，overlay 和 PLC 前最终结果随同一 `can_grab` 状态收敛。验证通过抓取限位定向测试、overlay 定向测试、Qt 主程序 Release 构建和完整默认测试脚本；未连接真实 PLC/相机/设备，现场仍需用有效九点标定人工确认叠层位置。
- 2026-08-13: 已修复当前机器 `bios_serial` 为空导致授权不符的问题。管理员授权规则已继续收敛为稳定机器身份：Windows MachineGuid、系统盘序列号和 Qt machineUniqueId 三项必须匹配；BIOS 序列号如果授权申请中非空也必须匹配；MAC 列表只作辅助记录，不再因网卡状态变化直接判整机不符。当前 `config/TankEye_admin_license_request_TK-B57A-B3ED-5AA9-3F90.json` 的 BIOS 为空，重新生成的 `config/admin_license.json` 和 `build/Release/config/admin_license.json` 已同步，hash 一致。验证通过授权测试、Qt 主程序和授权工具 Release 构建、完整默认测试脚本；未启动真实 GUI。
- 2026-08-13: 已生成 `TankEye-Iris_2.1.4` 运行包和 ZIP。源码配置、应用默认版本、打包脚本、运行说明和打包说明同步到 `2.1.4`；产物为 `dist/TankEye-Iris_2.1.4` 与 `dist/TankEye-Iris_2.1.4.zip`。本包包含夹爪框整框保护区硬约束、管理员授权空 BIOS 修复和当前源码状态；通用包包含 `config/admin_auth.key`，不携带单机 `admin_license.json`。验证通过 Release 主程序/设备探测程序/授权工具/授权测试/抓取限位/overlay 构建、完整默认测试、正式打包和包内关键文件/版本/hash/no-docs/no-AGENTS/no-license 检查；未启动包内 launcher，未连接真实 PLC、真实相机或真实设备。
- 2026-08-14: 已按“一台机器授权一次，后续直接登录”口径重新打包 `TankEye-Iris_2.1.4`。本次 2.1.4 包内管理员授权不再把 MAC 列表完全一致作为硬拒绝条件，避免退出再进因网卡状态变化显示 `授权文件不符`；稳定身份变化仍拒绝。验证通过授权定向测试、完整默认测试、重新打包和包内检查；未启动真实 GUI。
- 2026-08-27: 已实现 Qt 三档显示与 PLC 检测帧手动保存会话。主界面显示模式从二态 `隐藏/全显` 收敛为 `隐藏 -> 全显 -> 无显示` 循环，默认仍为 `隐藏`；`无显示` 只展示原始图，不绘制算法 overlay。新增 `开始保存/停止保存` 按钮，普通操作员和管理员界面均可使用；点击开始保存时锁定当前显示模式，后续只在 PLC 触发检测完成后保存该帧到程序目录 `images/`，文件名包含 `tankeye_plc_<时间戳>_<none|hidden|all>.png`。保存使用原始帧分辨率和现有 overlay 逻辑重新渲染，不改变算法、坐标转换、抓取过滤、PLC 寄存器或安全判断。验证通过 Qt 主程序 Release 构建、runtime status/overlay 定向测试、完整默认测试脚本、CPU + 模拟 PLC GUI 启动日志 `build/Release/logs/tankeye_20260827_142958.log`；未连接真实 PLC、真实相机或真实设备，未做真实 PLC 触发保存图片人工验收。
- 2026-08-27: 按用户进一步要求将显示模式入口从按钮循环改为下拉框，直接选择 `隐藏 / 全显 / 无显示`；保存会话与 PLC 检测帧留档逻辑保持不变，开始保存仍锁定当前下拉所选模式。界面新增 `displayModeSelector` 下拉样式，状态呈现测试改为断言下拉索引。验证通过 `tankeye-openvino_qt_app`、`tankeye-openvino_runtime_status_presenter_test` Release 构建、完整默认测试脚本，以及 CPU + 模拟 PLC GUI 启动日志 `build/Release/logs/tankeye_20260827_150043.log`；未连接真实 PLC、真实相机或真实设备，未做真实保存产物人工验收。
- 2026-08-27: 继续推进 `GraspMainWindow` 瘦身重构，把主窗体里已迁移到 `grasp_main_window_settings.cpp` 的设置、管理员、工程方案和授权入口逻辑彻底删回单一实现，并把 UI 缩放状态抽到 `grasp_main_window_scale.h` 统一共享。当前主窗体已不再保存那批设置/管理员重复实现，设置文件改为直接使用共享缩放 helper。已完成 Release 构建和完整默认测试脚本验证；后续还可继续把运行、PLC、渲染和保存职责再往外拆。
- 2026-08-27: 继续对 `GraspMainWindow` 做真瘦身，把运行、PLC、渲染、保存、目标列表和结果刷新从主窗体中拆到 `app/qt/ui/grasp_main_window_runtime.cpp`，并新增 `app/qt/ui/grasp_main_window_common.h` 统一显示模式文字、后缀和耗时 helper。当前主窗体约降到 2145 行，runtime 文件约 1643 行，主窗体从“大杂烩”进一步变成较薄编排壳。已完成 `tankeye-openvino_qt_app` Release 构建和完整默认测试脚本验证；`git diff --check` 仅有既有 LF/CRLF 提示，未连接真实 PLC/相机/设备。
- 2026-08-27: 继续把 `GraspMainWindow` 旧壳彻底清理掉，`app/qt/ui/grasp_main_window.cpp` 已从 CMake 构建列表移除并删除，职责分散到 `grasp_main_window_shell.cpp`、`grasp_main_window_runtime.cpp` 和 `grasp_main_window_settings.cpp`。当前 shell 约 1914 行、runtime 约 1480 行，`tankeye-openvino_qt_app` Release 构建、完整默认测试脚本和 CPU + 模拟 PLC 启动均通过；`git diff --check` 只剩仓内既有换行警告和本次修掉的 `CMakeLists.txt` EOF 空行问题。
