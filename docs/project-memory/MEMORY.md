...
- 2026-08-13: 已修复当前机器 `bios_serial` 为空导致授权不符的问题。管理员授权规则调整为：授权文件中记录的非空硬件指纹类必须全部匹配，且至少有 4 类有效绑定；MAC 列表仍完全一致匹配。当前 `config/TankEye_admin_license_request_TK-B57A-B3ED-5AA9-3F90.json` 的 BIOS 为空，重新生成的 `config/admin_license.json` 和 `build/Release/config/admin_license.json` 已同步，hash 一致。验证通过授权测试、Qt 主程序和授权工具 Release 构建、完整默认测试脚本；未启动真实 GUI。
