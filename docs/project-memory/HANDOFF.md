...
- 2026-08-06 latest package: `dist\TankEye-Iris_1.4.5` and `dist\TankEye-Iris_1.4.5.zip` were generated after updating package defaults/docs/config display version to 1.4.5. Verification passed: Release app/probe/settings builds, full default test script, package file/hash/UTF-8 checks, and direct packaged CPU + simulated PLC smoke log `dist\TankEye-Iris_1.4.5\logs\tankeye_pkg145_smoke_20260806_215728.log`, which confirms package-local settings plus OBB/SEG CPU compile and warmup. No real PLC/camera/device/reboot/GPU-success validation was run.
