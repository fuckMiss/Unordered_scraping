...
- 左上角显示：`name + V + version`，例如 `TankEye-Iris V2.1.1`。
- 顶部中间标题显示：`title`。
...
```powershell
powershell.exe -NoProfile -ExecutionPolicy Bypass -File .\scripts\package_runtime.ps1 -BuildDir build -ReleaseName TankEye-Iris_2.1.1 -Force
```
...
```text
dist\TankEye-Iris_2.1.1
dist\TankEye-Iris_2.1.1.zip
```

`dist\TankEye-Iris_2.1.1` 是可直接运行的文件夹，`dist\TankEye-Iris_2.1.1.zip` 是给新电脑拷贝使用的压缩包。运行包产物不纳入 GitHub。
...
```powershell
Test-Path .\dist\TankEye-Iris_2.1.1\tankeye-openvino_qt_app.exe
Test-Path .\dist\TankEye-Iris_2.1.1\tankeye-openvino_device_probe.exe
Test-Path .\dist\TankEye-Iris_2.1.1\platforms\qwindows.dll
Test-Path .\dist\TankEye-Iris_2.1.1\models\DG_8_weights\best_obb.xml
Test-Path .\dist\TankEye-Iris_2.1.1\models\DG_8_weights\best_seg.xml
Test-Path .\dist\TankEye-Iris_2.1.1\openvino_intel_cpu_plugin.dll
Test-Path .\dist\TankEye-Iris_2.1.1\openvino_intel_gpu_plugin.dll
Test-Path .\dist\TankEye-Iris_2.1.1\config\admin_auth.key
Test-Path .\dist\TankEye-Iris_2.1.1\USAGE_GUIDE.txt
Test-Path .\dist\TankEye-Iris_2.1.1\docs
Test-Path .\dist\TankEye-Iris_2.1.1\AGENTS.md
```
...
```powershell
(Get-FileHash .\build\Release\tankeye-openvino_qt_app.exe).Hash -eq (Get-FileHash .\dist\TankEye-Iris_2.1.1\tankeye-openvino_qt_app.exe).Hash
(Get-FileHash .\build\Release\tankeye-openvino_device_probe.exe).Hash -eq (Get-FileHash .\dist\TankEye-Iris_2.1.1\tankeye-openvino_device_probe.exe).Hash
```
...
确认 `config\admin_license.json` 存在、属于本机、由同一份 `config\admin_auth.key` 生成；允许 5 类硬件指纹中最多 1 类变化，变化 2 类及以上会判为无效。忘记密码时还需要回答创建管理员时设置的恢复问题，不能只凭授权文件直接重置。
...
```text
dist\TankEye-Iris_2.1.1\openvino_cache
```
