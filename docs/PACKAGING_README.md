...
正式包使用 `config\admin_auth.key` 验证管理员授权码。该文件不应提交到 GitHub；如果存在，打包脚本会复制进运行包。

新机器首次创建管理员账号时，让现场复制软件显示的机器码，然后生成 `INIT` 授权码：
...
如果脚本提示使用开发默认密钥，说明没有找到 `config\admin_auth.key`；正式打包前需要补齐密钥文件。
